...
import com.example.parcial_lab2.repositories.ArticuloInsumoRepository;
...
public class ArticuloInsumoService implements BaseService<ArticuloInsumo>{
    ...
    public ArticuloInsumo agregarImagenArticulo(Long idArticuloInsumo, Set<ImagenArticulo> imagenArticulo) throws Exception {
        try {
            ArticuloInsumo articuloInsimo = articuloInsumoRepository.findById(idArticuloInsumo).orElse(null);
            if (articuloInsimo != null) {
                articuloInsimo.setImagenArticulos(imagenArticulo);
                articuloInsumoRepository.save(articuloInsimo);
                return articuloInsimo;
            } else {
                return null;
            }
        } catch (Exception e){
